...
commands:
  ...
  - name: config unset
    ...
    args: KEY
  - name: aiflow create
    description: Create a NEW AIFlow (every run builds a fresh flow — DRAFTs are never reused). V2 pipeline upload+create+test_connection+save_pitch+get_prompt+regenerate+save_prompt+get_setting+time-template-pick+meeting-list+save_setting (DRAFT→ACTIVE). Pitch content is LLM-generated from --url. Per-step emailContent prompt templates are LLM-generated by default (--regenerate-emails=True). Launch is also default-on (--launch=True) — without save/setting the flow has no sequence bound and the scheduler can't send emails. Pass --no-launch only when you intend to populate prompts later via aiflow prompt-update. Launch is blocked with AIFLOW_LAUNCH_PROMPTS_INCOMPLETE when any step's content is still empty.
