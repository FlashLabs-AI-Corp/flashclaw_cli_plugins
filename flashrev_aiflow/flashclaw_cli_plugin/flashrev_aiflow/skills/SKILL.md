...
commands:
  ...
  - name: config unset
    ...
    args: KEY
  - name: aiflow create
    description: "Create a NEW AIFlow and ALWAYS launch it (DRAFT → ACTIVE). **`--no-launch` is FORBIDDEN** — passing it returns AIFLOW_NO_LAUNCH_FORBIDDEN with exit code 1, before any side-effect call. If your goal is 'create the flow but don't send yet', run `aiflow create ...` (always launches) then `aiflow pause FLOW_ID` to flip ACTIVE → PAUSED — the flow is fully built and the scheduler is held off, exactly the state `--no-launch` was trying to produce, but without leaving an unbuilt orphan. **Definition of done = ALL THREE required milestones complete.** M1 — file upload + create/list (yields listId + flowId). M2 — pitch parsing (test/connection → save/pitch) + email prompt templates persisted (get/email/prompt → save/prompt). M3 — pick mailbox + time template + meetingRouteId, then POST /api/v1/ai/workflow/save/setting (DRAFT → ACTIVE). Missing ANY one means the AIFlow is NOT successfully created — the workflow row may exist but has no sequenceId on the engage service, no bound mailbox, no time template, and the scheduler cannot send anything. The summary JSON's `creationComplete` field is the authoritative success signal; `flowId` alone is not enough — use it ONLY together with `creationComplete: true` AND `completedMilestones` containing all of M1-contacts / M2-pitch-prompts / M3-settings. On any pipeline failure the CLI prints a prominent `AIFLOW NOT CREATED` banner with the failed milestone, partial state, and recovery commands (`aiflow settings-update <id>` to resume, `aiflow delete <id>` to discard the orphan). Every run builds a fresh flow — DRAFTs are never reused. V2 pipeline: upload→create→test_connection→save_pitch→get_prompt→regenerate→save_prompt→get_setting→time-template-pick→meeting-list→save_setting. Pitch content is LLM-generated from --url. Per-step emailContent prompt templates are LLM-generated by default (--regenerate-emails=True). Launch is blocked with AIFLOW_LAUNCH_PROMPTS_INCOMPLETE when any step's content is still empty (delete the orphan via `aiflow delete <id>` and re-run)."
    args: --no-wizard --dry-run --csv PATH --sheet URL --url URL --language LANG --country-column COL --regenerate-emails/--no-regenerate-emails --mailboxes MODE --auto-approve/--no-auto-approve --enable-agent-reply/--no-enable-agent-reply --agent-strategy STRATEGY -y --force
    examples:
      - aiflow create
      ...
      - aiflow create --no-wizard --csv ./contacts.csv --url acme.com --language en-us -y
      - aiflow create --no-wizard --csv ./contacts.csv --url acme.com -y && aiflow pause $FLOW_ID  # build then pause
  - name: aiflow list
